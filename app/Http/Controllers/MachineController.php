...
class MachineController extends Controller
{
	...
	* 
	*/
	public function getMachineAnalyticsV2(Request $request)
	{		
		$header = getallheaders();
 		if(isset($header['orgId']) && ($header['orgId']!='') 
 			&& isset($header['projectId']) && ($header['projectId']!='')
		...
		//$mCntPer = ($mCnt / 100) * $mCnt;

		
		if ($mCnt > 0 ) {

			$mCntPer = ($mCnt / $mCnt) * 100;
	
			$resultData[0]['percentValue'] = $mCntPer;
			$resultData[0]['status'] = 'Total Machine ('.$mCnt.')';
			$resultData[0]['statusCode'] = 0;
		}
		$cnt = 1;
		foreach ($statusCode as $data) {
			...
			if ($approvedCnt > 0) {
				...
				$approvedCntPer = ($approvedCnt / $mCnt) * 100;

				$resultData[$cnt]['percentValue'] = number_format($approvedCntPer,2);
				$resultData[$cnt]['status'] = $statusName.'('.$approvedCnt.')';
				$resultData[$cnt]['statusCode'] = $data['statusCode'];
				$cnt ++;
			}
